...
var countLetters = () => {
  const result = wordlist.reduce((all, curr) => {
...
var letterCount = countLetters();
var letterCountArr = Object.entries(letterCount);
var alphOrder = letterCountArr.sort((a, b) => a[0].localeCompare(b[0]));
console.table(alphOrder);
var countOrder = letterCountArr.sort((a, b) => b[1] - a[1]);
console.log("Letter Count:");
console.table(countOrder.map((letter) => ({ Letter: letter[0], Count: letter[1] })));
var topLetters = countOrder.slice(0, 7);
var topWords = wordlist.filter((word) => {
  const letters = topLetters.map((l) => l[0]);
  const wordChars = word.split("");
  const inTopLetters = wordChars.every((char) => letters.includes(char));
  return inTopLetters && (/* @__PURE__ */ new Set([...wordChars])).size === 5;
});
